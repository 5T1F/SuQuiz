...
| 0    |          |                         | 2\. 마이페이지                                            |
| 0    |          |                         | 3\. 커뮤니티                                             |
| 0    | 마이페이지    | 마이페이지 - 내정보             | 1\. 경험치, 티어                                          |
| 0    | 로그인 페이지  | 로그인                     | 1\. 소셜로그인                                            |
| 0    |          | 회원가입                    | SNS와 연동                                              |
| 0    | 메인       | 메인페이지                   | 1\. 튜토리얼                                             |
| 0    |          |                         | 2\. 학습하기                                             |
| 0    |          |                         | 3\. 퀴즈 (퀴즈 시작 로비로 이동)                                |
| 0    | 학습       | 스테이지                    | 1\. 난이도별, 주제별 스테이지가 존재                               |
| 0    |          | 학습 영상                   | 1\. 국립국어원 API 활용하여 학습영상과 단어를 카드 형식으로 보여줌             |
| 0    |          |                         | 2\. 단어와 수어(사진/영상)를 카드 뒤집기 방식으로 전환                    |
| 0    |          |                         | 3\. 사용자 모션 인식 - 화상 캠 띄우기                             |
| 0    |          | 학습 모드                   | 1\. 학습 시에는 채팅 등 알림 비활성화 - 사용자 설정 여부                  |
| 0    | 퀴즈 시작 로비 | 퀴즈 시작 로비창               | 1\. 협동 모드                                            |
| 0    |          |                         | 2\. 대전 모드                                            |
| 0    |          |                         | 1\. 대전모드 버튼(퀵매치)                                     |
| 0    |          | 대전모드 - 게임 페이지           | 3-1. web RTC 화상 채팅 기능                                |
| 0    |          |                         | 3-2. 채팅 기능                                           |
| 0    |          | ※ 게임 진행중 순위 오른쪽 사이드에 표시 | 3-3. 참가인원이 모두 찼을때 자동 시작                              |
| 0    |          |                         | 3-4. 참가자들의 티어에 맞는 제시어 등장                             |
| 0    |          |                         | 3-5. 로테이션으로 돌아가며 그림을 그릴 수 있는 기능                      |
| 0    |          |                         | 3-6. 참가자의 행동 인식 후 정답 채점                              |
| 0    |          |                         | 3-7. 정답 시 점수 획득, 오답시 점수 차감                           |
| 0    |          | 대전모드 - 결과 페이지(모달창)      | 1\. 참가자가 해당 게임에서 획득한 점수 및 순위 표시                      |
| 0    |          |                         | 3\. 퀴즈 로비로 돌아가기                                      |
| 0    |          | 협동모드                    | 1-1. 주제 선택                                           |
| 0    |          | 협동모드 - 방 참가 선택          | 1-2. 난이도 선택                                          |
| 0    |          |                         | 1-3. 총 문제 개수 설정                                      |
| 0    |          |                         | 1-4. 문제당 시간제한 설정                                     |
| 0    |          |                         | 2\. 방 생성하기(초대 코드 생성)                                 |
| 0    |          |                         | 2-1. 방 참가하기(코드로 참가)                                  |
| 0    |          | 협동모드 - 게임 페이지           | 3-1. 3 2 1 카운트 후, 이미지(글) 제공                          |
| 0    |          |                         | 3-2. 플레이어 모두 동시에(1-2초) 이미지에 있는 수어 동작                 |
| 0    |          |                         | 3-3. 맞으면 O, 틀리면 X 화면 중앙에 표시                          |
| 0    |          |                         | 3-4. 맞췄으면 현재 맞춘 개수를 증가                               |
| 0    |          | 협동모드 - 결과 페이지           | 1\. 팀이 해당 게임에서 맞춘 문제 및 오답 수 표시                       |
| 0    |          |                         | 3\. 로비로 돌아가기                                         |
| 1    |          |                         | 2\. 업적                                               |
| 1    |          |                         | 2\. 문제 스트릭                                           |
| 1    |          |                         | 3\. 북마크된 문제                                          |
| 3    |          |                         | 4\. 최근 경기 기록                                         |
| 1    |          | 튜토리얼 제공                 | 선택한 이미지와 텍스트로 모드에 맞는 설명 제공                           |
| 1    |          |                         | 4\. teachable machine 을 통해 사용자가 학습영상과 일치한 동작을 하는지 판단 |
| 1    |          |                         | 5\. 동일 모션을 취하면 스테이지 통과                               |
| 2    | 커뮤니티     | 마이페이지 - 커뮤니티 및 채팅       | 1\. 친구 목록                                            |
| 2    |          |                         | 2\. 친구 추가                                            |
| 2    |          |                         | 3\. 친구 삭제                                            |
| 2    |          |                         | 4\. 친구 검색창                                           |
| 2    |          |                         | 5\. 1대1 채팅                                           |
| 3    |          |                         | 2\. 티어에 맞게 매칭                                        |
| 3    |          |                         | 2\. 순위에 따른 랭크점수 반등(상위 50퍼 점수상승, 하위 50퍼 점수하락)         |
| 3    |          |                         | 2\. 최근에 로그인 한 방식(마지막으로 로그인한 방식이 네이버인지 구글인지 표시해주는 기능) |
| 3    | 튜토리얼     | 학습 방법 튜토리얼              | 1\. 단어 제시                                            |
| 3    |          |                         | 2\. 단어 클릭 시 수어 제시                                    |
| 3    |          |                         | 3\. 수어 클릭 시 단어 제시                                    |
| 3    |          |                         | 4\. 학습 완료 시 다음 버튼 클릭                                 |
| 3    |          |                         | 5\. 다음 단어 학습 시작                                      |
| 3    |          | 게임 방법 튜토리얼              | 모드 선택                                                |
| 3    |          | \- 협동 모드 튜토리얼 (사설방)     | 0-1. 난이도, 주제 선택                                      |
| 3    |          |                         | 0-2. 초대 받은 사용자만 게임 참여                                |
| 3    |          |                         | 1\. 단어 제시                                            |
| 3    |          |                         | 2\. 준비 시간이 끝나면 단어에 맞는 수어 동작                          |
| 3    |          |                         | 3\. 참가자 모두 정답 시 점수 부여                                |
| 3    |          |                         | 4\. 게임 종료 후 최종 점수 산출                                 |
| 3    |          |                         | 5\. 점수에 따라 경험치 제공                                    |
| 3    |          |                         |                                                      |
| 3    |          | \- 대전 모드 튜토리얼 (퀵 매치)    | 1\. O명의 이용자가 돌아가면서 단어 설명                             |
| 3    |          |                         | 1-1. 설명 받는 이용자들은 단어를 수어로 맞춘다                         |
| 3    |          |                         | 해당 순서를 반복하고 점수 산출                                    |
| 3    |          |                         | 4\. 출제했던 단어/그림들 보여주기                                 |
| 3    |          |                         | 2\. 틀렸던 문제들 다시 보여주기                                  |

# ERD 설계

![SUQUIZ_ERD](/uploads/3c0fd269e19ae5a8481628bba802208c/SUQUIZ_ERD.png)
...
[기능 명세서](https://lab.ssafy.com/s10-webmobile1-sub1/S10P11B302/-/blob/master/docs/SUQUIZ_%EA%B8%B0%EB%8A%A5_%EB%AA%85%EC%84%B8%EC%84%9C.pdf?ref_type=heads)

[ERD](https://lab.ssafy.com/s10-webmobile1-sub1/S10P11B302/-/blob/master/docs/SUQUIZ_ERD.png)
