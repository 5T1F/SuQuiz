...
const Keyboard = (
  {
    ...
    keyboardMaps,
    handleKeyPress,
  } /*{ handleKeyPress, handleBackspace, handleEnter, inputString, rightGuess }*/
) => {
  ...
  useEffect(() => {
    //
    // keyboardArrays.reduce((x, element) => {
    //   element.reduce((x, ele) => {
    //     setKeyboardMaps((prev) => new Map(prev).set(ele, 0));
    //   }, new Map());
    // }, new Map());
  ...
  // 버튼을 렌더링하는 함수
  // const renderButtons = (keyboardMaps) => {
  //   return letters.map((letter, index) => (
  //     <button key={index} className="keyboard-col-white" onClick={() => keyClick(letter, index)}>
  //       {letter}
  //     </button>
  //   ));
  // };
  ...
  const renderButtons = (num) => {
    ...
    keyboardMaps.forEach((value, key) => {
      if (keyboardArrays[num].includes(key)) {
        buttons.push(
          <button key={key} className={`${getKeyCSS(value)}`} onClick={() => keyClick(key)}>
            {key}
          </button>
        );
      }
  ...
  const getKeyCSS = (status) => {
    if (status === 2) return "keyboard-col-green";
    else if (status === 1) return "keyboard-col-yellow";
    else return "keyboard-col-white";
  };
  ...
  return (
    ...
    <div>
      <div className="keyboard-row">{renderButtons(0)}</div>

      <div className="keyboard-row">
        {renderButtons(1)}{" "}
        <button onClick={clickEnter} className="keyboard-col-white">
          ⏎
        </button>
      </div>
      ...
      <div className="keyboard-row">
        {renderButtons(2)}
        <button onClick={handleBackspace} className="keyboard-col-white">
          ⌫
        </button>
      </div>
